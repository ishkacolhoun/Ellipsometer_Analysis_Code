...
def residual(x):
    d = x[0]
    Psi_pred = Psi(phi_0_DEG, d)
    Delta_pred = Delta(phi_0_DEG, d)
...
x0 = [ 3e-99 ] # initial guess for N_1 and d

#print(x0[2])

res = minimize(residual, x0,bounds=[(0,None)], method="Nelder-Mead")  
print(' d:',res.x[0])#,'\n d:',res.x[2])
...
Delta_plot = Delta(AoI_range, res.x[0])
Psi_plot = Psi(AoI_range, res.x[0])
